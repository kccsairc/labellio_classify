...
def main(args):
    ...
    for image in images:
        results = result(checkpoint_path = os.path.join(model_dir,"model.ckpt-{}".format(iteration)),
                        model_name = network_name,
                        num_classes = num_classes,
                        img_path = os.path.join(image_dir, image))
        tmp = results[np.argsort(results,axis=0)[::-1][0,1]]
        print("{}\t{}\t{}".format(os.path.join(image_dir, image),label_number_name[str(int(tmp[0]))],tmp[1]))

def get_parser():
    import argparse

    parser = argparse.ArgumentParser(description="Labellio Classifier")
    parser.add_argument("model_dir", help="(exp. model-1000 :this number is the number of iteration) Path to a model directory which is exported from Labellio. (Please extract the archive before you use it.)")
